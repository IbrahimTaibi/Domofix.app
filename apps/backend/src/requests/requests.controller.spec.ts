...
describe('RequestsController', () => {
  ...
  const serviceMock = {
    createRequest: jest.fn(async (userId: string, dto: any) => ({ id: 'r1', userId, ...dto })),
    applyForRequest: jest.fn(async (userId: string, id: string, dto: any) => ({ id, appliedBy: userId, ...dto })),
    acceptProvider: jest.fn(async (userId: string, id: string, dto: any) => ({ id, acceptedBy: userId, ...dto })),
    completeRequest: jest.fn(async (userId: string, id: string) => ({ id, completedBy: userId })),
  ...
  it('apply uses provider userId', async () => {
    ...
    expect(serviceMock.applyForRequest).toHaveBeenCalledWith('p1', 'r1', { message: 'Ready' });
    expect(res).toMatchObject({ id: 'r1', appliedBy: 'p1' });
  });

  it('acceptProvider uses customer userId', async () => {
    const req: any = { user: { userId: 'c1' } };
    const res = await controller.acceptProvider(req, 'r1', { providerId: 'p1' } as any);
    expect(serviceMock.acceptProvider).toHaveBeenCalledWith('c1', 'r1', { providerId: 'p1' });
