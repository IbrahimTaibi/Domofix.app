...
export class SanitizeInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): ReturnType<CallHandler['handle']> {
    ...
    if (req) {
      ...
      if (q && typeof q === 'object' && req.query && typeof req.query === 'object') {
        try {
          // Remove keys that were stripped by sanitizer
          Object.keys(req.query).forEach((k) => {
            if (!(k in q)) delete (req.query as any)[k];
          });
          Object.assign(req.query, q);
        } catch {
          // If mutation fails, attach sanitized copy without touching the getter
          (req as any).sanitizedQuery = q;
        }
      }

      const p = sanitizeObject(req.params);
      if (p && typeof p === 'object' && req.params && typeof req.params === 'object') {
        try {
          Object.keys(req.params).forEach((k) => {
            if (!(k in p)) delete (req.params as any)[k];
          });
          Object.assign(req.params, p);
        } catch {
          (req as any).sanitizedParams = p;
        }
      }
    }
    return next.handle();
