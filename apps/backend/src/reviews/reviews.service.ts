...
import type { Review as ReviewDTO, Comment as CommentDTO } from '@darigo/shared-types'
import { NotFoundError, AuthorizationError, DatabaseError } from '@/common/errors/app-error'
...
export class ReviewsService {
  ...
  async create(authorUserId: string, dto: CreateReviewDto): Promise<ReviewDTO> {
    try {
      ...
      this.logger.info('ReviewCreated', { reviewId: entity.id, providerId: entity.providerId })
      ...
      throw new DatabaseError('Failed to create review', undefined, { cause: err })
    }
  }

  async list(query: ListReviewsQueryDto): Promise<{ data: ReviewDTO[]; total: number; page: number; limit: number }> {
    ...
    return { data, total, page, limit }
      ...
      throw new DatabaseError('Failed to list reviews', undefined, { cause: err })
    }
  }
  ...
  async get(id: string): Promise<ReviewDTO> {
    try {
      ...
      throw new DatabaseError('Failed to fetch review', undefined, { cause: err })
    }
  }

  async update(id: string, userId: string, dto: UpdateReviewDto): Promise<ReviewDTO> {
    try {
      const doc = await this.reviewModel.findById(new Types.ObjectId(id))
      if (!doc) throw new NotFoundError('Review not found')
      if (doc.customerId.toString() !== userId) throw new AuthorizationError('Not allowed')
      ...
      if (err instanceof NotFoundError || err instanceof AuthorizationError) throw err
      throw new DatabaseError('Failed to update review', undefined, { cause: err })
    }
  }
  ...
  async delete(id: string, userId: string): Promise<void> {
    try {
      ...
      if (err instanceof NotFoundError || err instanceof AuthorizationError) throw err
      throw new DatabaseError('Failed to delete review', undefined, { cause: err })
    }
  }

  async addComment(authorUserId: string, dto: CreateCommentDto): Promise<CommentDTO> {
    const reviewId = new Types.ObjectId(dto.reviewId)
    const parentId = dto.parentCommentId ? new Types.ObjectId(dto.parentCommentId) : undefined
    try {
      ...
      this.logger.info('CommentAdded', { reviewId: dto.reviewId, commentId: (doc._id as any).toString() })
      return {
      id: (doc._id as any).toString(),
      reviewId: (doc.reviewId as any).toString(),
      parentCommentId: doc.parentCommentId ? (doc.parentCommentId as any).toString() : undefined,
      ...
      throw new DatabaseError('Failed to add comment', undefined, { cause: err })
    }
  }
  ...
  async listComments(reviewId: string, query: ListCommentsQueryDto) {
    ...
    if (query.parentCommentId) filter.parentCommentId = new Types.ObjectId(query.parentCommentId)
    try {
      const [items, total] = await Promise.all([
        this.commentModel
          .find(filter)
      ...
      parentCommentId: i.parentCommentId ? (i.parentCommentId?.toString?.() ?? String(i.parentCommentId)) : undefined,
    ...
    return { data, total, page, limit }
      ...
      throw new DatabaseError('Failed to list comments', undefined, { cause: err })
    }
  }
  ...
  private async safeNotify(userId: string, payload: { title: string; message: string; severity: 'info' | 'success' | 'warning' | 'error'; type: any; data?: Record<string, unknown> }) {
    try {
