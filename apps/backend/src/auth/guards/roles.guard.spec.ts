...
describe('RolesGuard', () => {
  ...
  const reflector = { getAllAndOverride: jest.fn((key: string) => (key === 'roles' ? getHandlerRoles() ?? getClassRoles() : undefined)) } as unknown as Reflector
  const guard = new RolesGuard(reflector)

  const ctx: any = {
    switchToHttp: () => ({ getRequest: () => ({ user: { role: 'provider' } }) }),
    getHandler: () => ({}),
    getClass: () => ({}),
  }
