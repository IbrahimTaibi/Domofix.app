...
export class ProviderApplicationsEventsListener {
  ...
  async handleProviderApplicationCreated(
    payload: ProviderApplicationCreatedPayload,
  ) {
    try {
      ...
      if (!toEmail) {
        this.logger.warn(
          'Skipping providerApplication.created email: no user email',
          {
      ...
      await this.emailService.sendProviderApplicationReceivedEmail(
        toEmail,
        payload.id,
      );
    } catch (err: any) {
